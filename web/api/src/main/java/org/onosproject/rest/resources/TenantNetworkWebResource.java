...
public class TenantNetworkWebResource extends AbstractWebResource {
    ...
    public Response getNetworks(@QueryParam("id") String queryID,
        ...
        log.info("Query completion. Return the size of network is {}",
                 networksMap.size());
    ...
    public Response getNetwork(@PathParam("id") String id) {
        log.info("Starting query network configuration information by identifier {}.",
                 id);
        TenantNetwork network = nullIsNotFound(get(TenantNetworkService.class)
                                                        .getNetwork(TenantNetworkId
                                                                            .networkId(id)),
                                                NETWORK_NOT_FOUND);
        log.info("Query completion. Return the json");
        ObjectNode result = new ObjectMapper().createObjectNode();
    ...
    public Response deleteNetworks(@PathParam("id") String id, InputStream input)
            throws IOException {
        try {
            log.info("Starting delete network.", id);
    ...
    public Response createNetworks(InputStream input) throws IOException {
        try {
            log.info("Starting create network.");
            ObjectMapper mapper = new ObjectMapper();
            JsonNode cfg = mapper.readTree(input);
            JsonNode nodes = null;
    ...
    public Response updateNetworks(@PathParam("id") String id, InputStream input)
            throws IOException {
        try {
            log.info("Starting update network. identifier {}", id);
            ObjectMapper mapper = new ObjectMapper();
            JsonNode cfg = mapper.readTree(input);
            JsonNode nodes = null;
            Iterable<TenantNetwork> networks = null;
